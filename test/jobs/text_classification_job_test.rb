...
class TextClassificationJobTest < ActiveJob::TestCase
  ...
  test "job result in execution classify article content" do
    dummy_text = 'hello Keanu Reeves, You are breathtaking. You are amazing. You are gorgeous' \
                 'You are simply the best. See you soon in Cyberpunk'
    sample_data = {
      categories: [
  ...
  test "error job result in execution classify article content for invalid word count" do
    ...
    assert_performed_jobs 0, only: TextClassificationJob do
      Google::GoogleCloudLanguage.stub(:new, @mock) do
        @new_article = create :article, content: dummy_text
      end
    end
    assert_nil @new_article.reload.category
  end
end
